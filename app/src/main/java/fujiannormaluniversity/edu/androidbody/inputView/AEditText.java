...
//    private int linePosition;
    ...
    private void onCreateView(Context context, AttributeSet attrs) {
      ...
      onCustomMethod(context,attrs);
    }

    private void onCustomMethod(Context context,AttributeSet attrs) {

        // 获取控件资源
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.AEditText);
        try {
            ...
            // a. 点击状态的左侧图标
            // 1. 获取资源ID
            ic_left_clickResID = typedArray.getResourceId(R.styleable.AEditText_ic_left_click, 0);

            if ( ic_left_clickResID != 0 ){

                // 2. 根据资源ID获取图标资源（转化成Drawable对象）
                ic_left_click =  OSHelper.getDrawable(context,ic_left_clickResID);

                // 3. 设置图标大小
                // 起点(x，y)、宽= left_width、高 = left_height
                left_x = typedArray.getInteger(R.styleable.AEditText_left_x, 0);
                left_y = typedArray.getInteger(R.styleable.AEditText_left_y, 0);
                left_width = typedArray.getInteger(R.styleable.AEditText_left_width, 60);
                left_height = typedArray.getInteger(R.styleable.AEditText_left_height, 60);
            ...
            // b. 未点击状态的左侧图标
            // 1. 获取资源ID
            ic_left_unclickResID = typedArray.getResourceId(R.styleable.AEditText_ic_left_unclick, 0);

            if (ic_left_unclickResID != 0){
                // 2. 根据资源ID获取图标资源（转化成Drawable对象）
                // 3. 设置图标大小（此处默认左侧图标点解 & 未点击状态的大小相同）
                ic_left_unclick =  OSHelper.getDrawable(context,ic_left_unclickResID);
            ...
            // 1. 获取资源ID
            ic_deleteResID = typedArray.getResourceId(R.styleable.AEditText_ic_delete,0);
        // 2. 根据资源ID获取图标资源（转化成Drawable对象）
            Bitmap bitmap;
            if (ic_deleteResID == 0){
                ...
                ic_delete =  OSHelper.getDrawable(context,ic_deleteResID);
                BitmapDrawable bitmapDrawable = (BitmapDrawable) ic_delete;
                bitmap = bitmapDrawable.getBitmap();

            }
            // 3. 设置图标大小
            // 起点(x，y)、宽= left_width、高 = left_height
            delete_x = typedArray.getInteger(R.styleable.AEditText_delete_x, 0);
            delete_y = typedArray.getInteger(R.styleable.AEditText_delete_y, 0);


            delete_width = typedArray.getInteger(R.styleable.AEditText_delete_width, bitmap.getWidth() );
            delete_height = typedArray.getInteger(R.styleable.AEditText_delete_height, bitmap.getWidth() );
            ...
            // 原理：通过 反射机制 动态设置光标
            // 1. 获取资源ID
            cursor = typedArray.getResourceId(R.styleable.AEditText_cursor, 0);
            if (cursor != 0){
                try {
            ...
            int lineColorClick_default = OSHelper.getColor(context, R.color.main); // 默认 = 蓝色#1296db
            int lineColorunClick_default = OSHelper.getColor(context,R.color.border); // 默认 = 灰色#9b9b9b
            lineColor_click = typedArray.getColor(R.styleable.AEditText_lineColor_click, lineColorClick_default);
            lineColor_unclick = typedArray.getColor(R.styleable.AEditText_lineColor_unclick, lineColorunClick_default);
            ...
            // 3. 分割线位置
            linePosition = typedArray.getInteger(R.styleable.AEditText_linePosition, 1);
